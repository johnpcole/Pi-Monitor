...
###############
# By: Nitzan (http://blog.backslasher.net/python-library-for-deluge-torrent-maintenance.html)
...
from deluge.ui.client import DClient
    ...
    for torrent_id,torrent_info in torrents.items():
        try:
            ...
            if res == 'd':
                ...
                tlist.append(DClient.core.remove_torrent(torrent_id, False).addCallbacks(printSuccess, printError, callbackArgs = (True, successmsg), errbackArgs = (errormsg)))
            elif res == 'D':
                ...
                tlist.append(DClient.core.remove_torrent(torrent_id, True).addCallbacks(printSuccess, printError, callbackArgs = (True, successmsg), errbackArgs = (errormsg)))
            elif res == 'l':
...
def on_session_state(result):
    DClient.core.get_torrents_status({"id": result}, torrent_info_wanted).addCallback(on_torrents_status)

def on_connect_success(result):
    printSuccess(None, True, "Connection was successful!")
    DClient.core.get_session_state().addCallback(on_session_state)
...
def filter_torrents(connection_data={},info_wanted=[],action=(lambda tid,tinfo: 'l'),interactive=True):
    ...
    # set parameters
    global cliconnect
    cliconnect = DClient.connect(**connection_data)
